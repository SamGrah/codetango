...
**Feature budget (near-zero JS).** Two deliberate exceptions, everything else JS-free: (1) `<ClientRouter/>` (~5.4 KB gzipped, global) for guaranteed-smooth page transitions; (2) **Pagefind search** (restored July 2026, MkDocs-style): the header magnifier (or ⌘K) opens a modal with live-as-you-type results — the *one* search field, on every page. Its component script rides in the head site-wide but idles until used; the index lazy-loads on first keystroke; scoped by `data-pagefind-body`; themed to site tokens incl. the accent `mark` highlight. Budget check: `find dist -name '*.js' -not -path '*/pagefind/*' | wc -l` = **1**; any further script must clear a real bar. Theme toggle is a binary override persisted to localStorage; system preference is the default until first click. Tag/category pages are not rendered (data preserved in frontmatter + RSS).
